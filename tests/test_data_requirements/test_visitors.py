...
class TestVisitorsDataRequirements(unittest.TestCase):
    ...
    def test_visitors_dni_unique(self):
        ...
        try:
            ...
            self.cursor.execute("INSERT INTO visitors (DNI, name, address, profession, accommodation_id) VALUES ('TEST12345678', 'Test Name', 'Test Address', 'Test Profession', %s)", (self.accommodation_id,))
            self.connection.commit()

            # Try inserting another visitor record with the same DNI
            with self.assertRaises(pymysql.err.IntegrityError):
                self.cursor.execute("INSERT INTO visitors (DNI, name, address, profession, accommodation_id) VALUES ('TEST12345678', 'Another Test Name', 'Another Test Address', 'Another Test Profession', %s)", (self.accommodation_id,))
                self.connection.commit()
    ...
    def test_visitors_data_insertion(self):
        ...
        try:
            # Insert valid data
            self.cursor.execute("INSERT INTO visitors (DNI, name, address, profession, accommodation_id) VALUES ('TEST98765432', 'Valid Name', 'Valid Address', 'Valid Profession', %s)", (self.accommodation_id,))
            self.connection.commit()
    ...
    def test_visitors_required_fields_not_null(self):
        ...
        try:
            ...
            with self.assertRaises(pymysql.err.IntegrityError):
                self.cursor.execute("INSERT INTO visitors (DNI, name, address, profession, accommodation_id) VALUES (NULL, 'Null DNI Name', 'Null DNI Address', 'Null DNI Profession', %s)", (self.accommodation_id,))
                self.connection.commit()
            self.connection.rollback()

            # Try inserting data with NULL name
            with self.assertRaises(pymysql.err.IntegrityError):
                self.cursor.execute("INSERT INTO visitors (DNI, name, address, profession, accommodation_id) VALUES ('TEST45678901', NULL, 'Null Name Address', 'Null Name Profession', %s)", (self.accommodation_id,))
                self.connection.commit()
            self.connection.rollback()
    ...
    def test_visitors_foreign_key_enforcement(self):
        ...
        try:
            ...
            self.cursor.execute("INSERT INTO visitors (DNI, name, address, profession, accommodation_id) VALUES ('TEST55555555', 'Invalid Name', 'Invalid Address', 'Invalid Profession', 999)")
            self.connection.commit()
